...
// Database/Seeders/InvoiceTemplatesSeeder.php
...
class InvoiceTemplatesSeeder extends Seeder
{
    ...
    {
        // Create system templates for each tenant
        $tenants = Tenant::all();
        
        foreach ($tenants as $tenant) {
            $this->createSystemTemplatesForTenant($tenant->id);
        }
    }
    
    private function createSystemTemplatesForTenant(string $tenantId): void
    {
        // Temporarily disable global scopes to create templates for specific tenant
...
// Updated API Service for Vue.js frontend to work with new domain structure
// resources/js/api/templates.js
export const templateApi = {
  async getTemplates() {
    return await axios.get('/api/templates/invoice')
  },

  async getTemplate(templateName) {
    return await axios.get(`/api/templates/invoice/${templateName}`)
  },

  async saveTemplate(templateData) {
    return await axios.post('/api/templates/invoice', templateData)
  },

  async updateTemplate(templateId, templateData) {
    return await axios.put(`/api/templates/invoice/${templateId}`, templateData)
  },

  async deleteTemplate(templateId) {
    return await axios.delete(`/api/templates/invoice/${templateId}`)
  },

  async previewTemplate(previewData) {
    return await axios.post('/api/templates/invoice/preview', previewData)
  }
}

export const invoiceApi = {
  async generateInvoice(invoiceId, options = {}) {
    return await axios.post(`/api/invoices/${invoiceId}/generate`, options, {
      responseType: 'blob' // For PDF download
    })
  }
}

// Service Provider to bind domain services
...
class TemplateServiceProvider extends ServiceProvider
{
    ...
    {
        // Services are automatically registered via $singletons property
    }
