...
**Weryfikacja lokalna (2026-07-02, Sail/Docker):** uruchomiono pełny `composer larastan` i `php artisan test` po ostatnich commitach review — szczegóły w [`REVIEW_LOCAL_QUALITY_RUN.md`](REVIEW_LOCAL_QUALITY_RUN.md). **PHPStan: 3 błędy** (`ApprovalResolutionService` — `HasMany::active()`). **PHPUnit: 251 failed / 55 passed** — dominująca przyczyna: migracja backfill `2025_07_10_000030_...` odwołuje się do nieistniejącej tabeli `contacts` na świeżej bazie testowej, więc `RefreshDatabase` pada w `setUp()` większości testów. Migracja backfill (punkt 8) **wymaga poprawki przed merge**; na kopii produkcyjnej dodatkowo przetestować logikę `DISTINCT ON`/`user_tenants` (PostgreSQL).
...
**Testy/PHPStan:** wcześniej brak `vendor/` w środowisku review — tylko `php -l`. **2026-07-02 (rano):** lokalnie CS Fixer + PHPStan OK (przed ostatnimi commitami Fazy 2). **2026-07-02 (wieczór, po `git pull`):** pełna weryfikacja w Sail — **`composer larastan` → 3 błędy** (`ApprovalResolutionService`), **`artisan test` → 251 failed / 55 passed** (blokada: migracja backfill `contacts`). Logi: [`REVIEW_LOCAL_QUALITY_RUN.md`](REVIEW_LOCAL_QUALITY_RUN.md) · [`phpstan.txt`](REVIEW_LOCAL_QUALITY_RUN/phpstan.txt) · [`artisan-test.txt`](REVIEW_LOCAL_QUALITY_RUN/artisan-test.txt). **Merge zablokowany do naprawy PHPStan + migracji backfill.**
