...
## Naprawione: Faza 5 critical — allocation dimensions + attachment authorization (2026-07-03)

Weryfikacja: `php -l` na każdym zmienionym pliku (0 błędów). Brak środowiska do faktycznego wywołania endpointów w tej sesji — logika sprawdzona ręcznie przeciw kodowi wołającemu (`DimensionItemResource`, `DimensionConfigurationController::getDimensionData()`, `AllocateExpenseAction::validateDimensionExists()`).

1. **9/11 klas wymiaru alokacji nie implementowało `AllocationDimensionInterface`** (jedyny wciąż otwarty CRITICAL z całego audytu) — `DimensionItemResource::toArray()` bezwarunkowo woła `getId/getCode/getName/getDescription/isGlobal/getDisplayName/getIsActive`, więc każda alokacja z wymiarem innym niż TRANSACTION_TYPE/STRUCTURE kończyła się `Error`. Naprawione:
   - 7 prostych modeli-słowników (`AllocationLocation`, `AllocationProductCategory`, `AllocationRevenueType`, `AllocationCostType`, `AllocationRelatedTransactionCategory`, `AllocationContractType`, `AllocationEquipmentType`) — dodano `implements AllocationDimensionInterface` + istniejący trait `HasAllocationDimensionInterface`, dokładnie ten sam wzorzec co już działający `AllocationTransactionType`.
   - `User` (wymiar Employees) — też przez trait, ale `getTenantId()` jest już zdefiniowane na klasie (JWT-owy kontekst tenanta bieżącego requestu, używany w ~50 plikach rdzenia auth/multi-tenancy) — metoda z traita jest przez to po cichu przesłonięta (to samo zachowanie co przed zmianą, nic nowego nie psuje; przemianowanie tej metody wykracza poza zakres tego fixa).
   - `Project` (wymiar Project) — przez trait, ale z override'em `getIsActive()`: Project nie ma kolumny `is_active` (używa `SoftDeletes`), więc zamiast fallbacku traita (`$this->is_active ?? true`, zawsze `true`) sprawdza `null === $this->deleted_at`.
2. **6/7 kontrolerów załączników nie wołało żadnej Policy** — sprawdzały tylko integralność (`media.model_id` należy do rekordu), nie czy user ma prawo zarządzać załącznikami TEGO rekordu. Naprawione tam, gdzie domena już miała Policy z regułami węższymi niż "ten sam tenant", a mimo to nie była wołana: `ProjectAttachmentsController`/`TaskAttachmentsController` (właściciel/przypisany członek — `ProjectPolicy`/`TaskPolicy`) i `InvoiceAttachmentsController` (`InvoicePolicy`). **Nienaprawione świadomie:** `Contractor`/`Expense`/`Product` — te domeny nie mają żadnej Policy dla głównego rekordu (własne kontrolery zasobów gate'ują tylko destroy/export przez `authorizeManage()` Owner/Admin, reszta CRUD jest celowo otwarta dla każdego membera tenanta) — dopisanie autoryzacji tutaj wymaga decyzji produktowej (jaka reguła?), nie jest to prosty "wpiąć istniejącą Policy" fix jak w pozostałych trzech.
3. **[MEDIUM z audytu, rozszerzony] Brak whitelisty MIME na uploadzie załączników — stored XSS.** Audyt wskazał Contractors/Products jako przykład, ale identyczna ekspozycja jest we wszystkich 7 domenach: `show()`/`preview()` serwują pliki z `Content-Disposition: inline` używając mime_type podanego przez uploadera, więc upload SVG/HTML z `<script>` renderuje się inline w originie aplikacji. Dodano współdzielony trait `HasAttachmentMimeWhitelist` (PDF/obrazy/Office/csv/txt/zip, bez `image/svg+xml`/`text/html`) i regułę `mimetypes:` do wszystkich 7 `*AttachmentRequest`.
