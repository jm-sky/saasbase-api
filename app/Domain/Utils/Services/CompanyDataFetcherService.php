...
use App\Domain\Utils\Jobs\MfLookupJob;
use App\Domain\Utils\Jobs\RegonLookupJob;
use App\Domain\Utils\Jobs\ViesLookupJob;
...
class CompanyDataFetcherService
{
    ...
    public function fetch(CompanyContext $context): ?AllLookupResults
    {
        ...
        if (($context->nip || $context->regon) && config('services.regon.user_key')) {
            $jobs[] = new RegonLookupJob($context);
        }

        if ($context->nip) {
            $jobs[] = new MfLookupJob($context);
        }

        if ($context->nip && $context->country) {
            $jobs[] = new ViesLookupJob($context);
        }
