...
class TenantScopedRoles
{
     ...
     * @return list<string>
     */
    public static function roleNamesFor(User $user, string $tenantId): array
    {
        return DB::table('model_has_roles')
            ->join('roles', 'roles.id', '=', 'model_has_roles.role_id')
            ->where('model_has_roles.model_id', $user->id)
    ...
    /**
     * Direct grants + permissions carried by tenant-scoped roles.
     *
     * @return list<string>
     */
    public static function permissionNamesFor(User $user, string $tenantId): array
    {
        ...
        return $direct->merge($viaRole)->unique()->sort()->values()->all();
    }
