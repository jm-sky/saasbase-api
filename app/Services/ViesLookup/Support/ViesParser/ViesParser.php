...
class ViesParser
{
    ...
    public function getParsedAddress(): ?ViesAddress
    {
        ...
        */
        ...
        if ($this->newlines == 1 && in_array($this->countryCode, ['NL', 'BE', 'FR', 'FI', 'AT', 'PL', 'DK'])) { // Countries in expected format
            return $this->parseEuropeanGroup1();
        }

        // Slovenia has everything on one line, split by comma, but seems fairly regular
        if ($this->newlines == 0 && in_array($this->countryCode, ['SI', 'HR'])) {
            return $this->parseEuropeanGroup2();
        }

        if ($this->newlines == 0 && in_array($this->countryCode, ['EL'])) {
            return $this->parseGreekGroup();
        }

        // Romania new format
        if ($this->newlines == 0 && in_array($this->countryCode, ['RO']) && str_contains($this->address, ',') && str_contains($this->address, 'SECTOR') && str_contains($this->address, 'STR')) {
            return $this->parseRomaniaGroup1();
        }

        // Romania does not have ZIP codes in VIES data
        if ($this->newlines == 1 && in_array($this->countryCode, ['RO'])) {
            return $this->parseRomaniaGroup2();
        }

        // Romania does not have ZIP codes in VIES data
        // With 3 lines, it has apartement in the last line - we put it on the start of street line
        if ($this->newlines == 2 && in_array($this->countryCode, ['RO'])) {
            return $this->parseRomaniaGroup3();
        }

        if ($this->newlines == 1 && in_array($this->countryCode, ['IT'])) {
            return $this->parseItalianGroup();
        }

        if ($this->newlines == 2 && in_array($this->countryCode, ['PT'])) {
        ...
        if ($this->newlines == 2 && in_array($this->countryCode, ['FR'])) {
            $address_split = explode("\n", $this->address);
            $street = $address_split[0].', '.$address_split[1];
            [$zip, $city] = explode(' ', $address_split[2], 2);
        ...
        if ($this->newlines == 2 && in_array($this->countryCode, ['SK'])) { // Vetsina SK address
            $address_split = explode("\n", $this->address);
            $street = $address_split[0];
            [$zip, $city] = explode(' ', $address_split[1], 2);
        ...
        if ($this->newlines == 1 && in_array($this->countryCode, ['SK'])) { // vetsinou ma tenhle format Bratislava
            $address_split = explode("\n", $this->address);
            $street = $address_split[0];

            if ($address_split[1] === 'Slovensko') {
                [$zip, $city] = explode(' ', $address_split[0], 2);
        ...
        if ($this->newlines == 0 && in_array($this->countryCode, ['EE']) && strpos($this->address, '  ') !== false) {
        ...
        if ($this->newlines == 1 && in_array($this->countryCode, ['CZ'])) { // Countries in expected format
            $address_split = explode("\n", $this->address);
            $street = $address_split[0];
            ...
            [$zip, $city] = [substr($address_split[1], 0, $pos), substr($address_split[1], $pos)];
        ...
        if ($this->newlines == 2 && in_array($this->countryCode, ['CZ'])) { // Countries in expected format
            $address_split = explode("\n", $this->address);
            $street = $address_split[0].', '.$address_split[1];
            ...
            [$zip, $city] = [substr($address_split[2], 0, $pos), substr($address_split[2], $pos)];
    ...
    protected function parseEuropeanGroup2()
    {
        ...
        [$zip, $city] = explode(' ', trim($address_split[array_key_last($address_split)]), 2);
